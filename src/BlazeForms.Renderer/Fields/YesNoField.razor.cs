...
/// <summary>
/// A yes-or-no choice presented as a two-option group (PRD §5,
/// <see cref="Definitions.NodeType.YesNo"/>). Like <see cref="RadioGroupField"/>, it renders
/// each <see cref="Definitions.FormOption.Label"/> but stores each option's
...
/// <see langword="null"/> when unanswered. Kept as its own component (rather than a mode of
/// <see cref="RadioGroupField"/>) so a host's <c>IFieldComponentRegistry</c> can swap the two
/// node types independently — a yes/no toggle and an open-ended radio group are different
/// design-system components in most hosts.
/// </summary>
/// <remarks>
/// <b>Accessibility.</b> The two options sit in a <c>fieldset</c>/<c>legend</c> for visual grouping
/// and carry a <c>role="radiogroup"</c> container named from the legend via <c>aria-labelledby</c>
...
public sealed partial class YesNoField : FormFieldBase
{
