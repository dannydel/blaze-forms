// Collocated ES module for KeyboardHelpDialog.razor's focus trap (PRD §4.1, §11).
//
...
// conditional per key value -- the same platform gap MoveToPositionDialog.razor.js and
// DeleteProtectionDialog.razor.js both document for their own traps. This listener calls
// preventDefault() only in the two cases it is actually about to redirect focus itself, and never
// stops propagation, so Escape (handled entirely on the Blazor side) still reaches the dialog's
// own @onkeydown handler untouched.
//
 ...
 * whose `dispose()` detaches it -- called from KeyboardHelpDialog.razor.cs's own DisposeAsync,
