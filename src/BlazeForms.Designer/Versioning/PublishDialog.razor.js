// Collocated ES module for PublishDialog.razor's focus trap (PRD §7, §11).
//
// The genuine platform gap this module fills: Tab and Shift+Tab must cycle focus among this
// dialog's own focusable controls -- the change-note textarea, zero or more blocker jump buttons,
// and the Confirm/Cancel buttons -- without ever letting it escape to the rest of the page while
...
// gap every other trapped dialog in this project documents for its own trap. This listener calls
...
function focusableElements(root) {
    return Array.from(root.querySelectorAll("select, button, input, textarea, [tabindex]"))
 ...
 * whose `dispose()` detaches it -- called from PublishDialog.razor.cs's own DisposeAsync,
