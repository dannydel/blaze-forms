// Collocated ES module for VisibilityRuleEditor.razor's focus trap (PRD §4.1, §6, §11).
//
// The genuine platform gap this module fills: Tab and Shift+Tab must cycle focus among this
// dialog's own focusable controls without ever letting it escape to the rest of the page while the
// dialog is open. Blazor's declarative @onkeydown:preventDefault modifier cannot be made
// conditional per key value -- there is no way to say "prevent the default only for Tab, and only
// when Tab would actually leave the dialog, but never touch any other key" -- the same platform gap
// MoveToPositionDialog.razor.js documents for its own focus trap (this module is a deliberate,
// collocated copy of that one rather than a shared import, matching AGENTS.md's "JS lives in
// collocated .razor.js ES modules" convention -- each dialog owns its own). This listener calls
 ...
 * whose `dispose()` detaches it -- called from VisibilityRuleEditor.razor.cs's own DisposeAsync,
